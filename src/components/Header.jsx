...
function Header() {
    ...
    function selectFilterType() {
        ...
        if (location.pathname == '/flights/transport/tickets/') {
            if (ticketFilterStatus) {
                return (
                    <div className='tr-filterFlight' >
                        <TicketFilter ticketFilterStatus={ticketFilterStatus} setTicketFilterStatus={setTicketFilterStatus} />
                        <FlightFilter />
                    </div>
                )
            }
            else {
                return <TicketFilter ticketFilterStatus={ticketFilterStatus} setTicketFilterStatus={setTicketFilterStatus} />
            }
        }
        if (location.pathname == '/flights/transport') {
            if (trFilterStatus) {
                return (
                    <div className='tr-filterFlight' >
                        <FlightFilter />
                ...
                return <TransportFilter setTrFilterStatus={setTrFilterStatus} />
            }
        }
    ...
    return (
        <>
            {trFilterStatus ? <div id='okk' onClick={() => setTrFilterStatus(false)} ></div> : null}
            <div className='header-bg' >
                <header className={`${location.pathname == '/flights/transport/tickets/' || location.pathname == '/carhire/results' ? 'container2 header2' : location.pathname == '/profile/saved' ? 'container3 header2' : 'container header'}`}>
                    <div>
                        <Link to={'/'} >
                        ...
                        <div>
                            ...
                            <div className='icons-bg'>
                                <TiWorld style={{ marginTop: '2px' }} size={27} color='white' />
                            </div>
                            <div className='icons-bg' onClick={() => goProfile('profile/saved')} >
                    ...
                    <nav>
                        ...
                        <NavLink to={'carhire'}>
                            <IoCarSharp size={20} color='white' />
