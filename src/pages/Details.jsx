...
import { data, Link, Navigate, useLocation, useNavigate, useParams } from 'react-router-dom';
import { getDetailsAboutTicket } from '../services/flightServices';
import Loader from '../components/Loader';
import { FaAlignLeft, FaAngleLeft, FaStar } from 'react-icons/fa';
import { getMonthName } from '../components/FlightFilter';
import { IoAirplane } from 'react-icons/io5';
import { MdAccessTimeFilled, MdFastfood } from 'react-icons/md';
...
function Details() {
    ...
    let departureTime = ticket?.departure?.slice(11)
    let arrivalTime = ticket?.arrival?.slice(11)
    let roundDepartureTime = ticket?.roundTrip?.departure.slice(11)
    let roundArrivalTime = ticket?.roundTrip?.arrival.slice(11)
    console.log(ticket);

    useEffect(() => {
        getDetailsAboutTicket(country)
            .then(
                data => setTicket(data[0].cities.find(item => item.name == city).flights.find(item => item.id == ID))
            )
    }, [])
        ...
        <>
            ...
            <section className='detail-section' >
                <div className="container">
                    <div className='about'>
                        <div className='col1' >
                            <h4 className='title-dt' >Подробнее о билете</h4>
                            <div>
                                {ticket.reservation.map((item, i) =>
                                    <div key={i} className='webSiteBox' >
                                        <div>
                                            <h4>{item.name}</h4>
                                            <div className='rating' >
                                                {Array.from({ length: item.rating }).map((_, i) =>
                                                    <FaStar key={i} fill='#f55d42' />
                                                )
                                                }
                                            </div>
                                        </div>
                        ...
                        <div className='col2' >
                            <h4 className='title-dt' >Информация о рейсе</h4>
                            <div>
                                <h5>Туда <span>{ticket.departure.slice(8, 10)} {getMonthName(ticket.departure.slice(5, 7))}. {ticket.departure.slice(0, 4)} y. </span></h5>
                                <div>
                                    <div className='info'>
                                        <div className='info-line' >
                                            <img src={`${ticket.image}`} alt={`${ticket.airline}`} />
                                            <div>
                                                <div>
                                                    <h5>{departureTime}</h5>
                                                    <p>{title1}</p>
                                    ...
                                    <div className='info2' >
                                        ...
                                        <div>
                                            ...
                                            <h4>{departureTime} {title1} {ticket.from}</h4>
                                            <h5><MdAccessTimeFilled size={17} /> {getFlightDuration(departureTime, arrivalTime)}</h5>
                                            <p><MdFastfood size={17} />Показать</p>
                                            <h4>{arrivalTime} {title2} {ticket.to}</h4>
                                        </div>
                                        <p><span>Прибытие: </span>{ticket.arrival.slice(8, 10)} {getMonthName(ticket.arrival.slice(5, 7))}. {ticket.arrival.slice(0, 4)} y.  |  <span>Время в пути: </span> {getFlightDuration(departureTime, arrivalTime)} </p>
                                    </div>
                                </div>
                                {ticket.roundTrip ? <h5>Обратно <span>{ticket.roundTrip.departure.slice(8, 10)} {getMonthName(ticket.roundTrip.departure.slice(5, 7))}. {ticket.roundTrip.departure.slice(0, 4)} y. </span></h5> : null}
                                {ticket.roundTrip ?
                                    <div>
                                        <div className='info'>
                                            <div className='info-line' >
                                        ...
                                        <div className='info2' >
                                            ...
                                            <div>
                                                ...
                                                <h4>{roundDepartureTime} {title2} {ticket.to}</h4>
                                                <h5><MdAccessTimeFilled size={17} /> {getFlightDuration(roundDepartureTime, roundArrivalTime)}</h5>
                                                <p><MdFastfood size={17} />Показать</p>
                                                <h4>{roundArrivalTime} {title1} {ticket.from}</h4>
                                            </div>
                                            <p><span>Прибытие: </span>{ticket.roundTrip.arrival.slice(8, 10)} {getMonthName(ticket.roundTrip.arrival.slice(5, 7))}. {ticket.roundTrip.arrival.slice(0, 4)} y.  |  <span>Время в пути: </span> {getFlightDuration(roundDepartureTime, roundArrivalTime)} </p>
                                        </div>
                                    </div> : null
                                }
                            </div>
